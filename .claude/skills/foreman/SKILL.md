...
If escalating, use the MCP tool `escalate`:

```
Tool: escalate
Arguments:
  question: "Your specific question"
  context: "Why you need help"
```
...
## Step 6: Complete Directive

After task planning is done, signal your completion:
...
- `cancel` -- Cancel a task. Terminates session and cleans up resources.
- `complete_directive` -- Signal that directive analysis/planning is complete. The directive will auto-complete when all tasks finish.
- `create_directive_tasks` -- Create the task DAG for a directive. Include tasks array with depends_on indices, acceptance_criteria, and decomposition_rationale.
- `create_subtasks` -- Split a task into replacement tasks (original cancelled). Use depends_on indices for inter-task dependencies.
- `escalate` -- Request human help when stuck. Provide a specific question. Session stays alive waiting for response.
- `escalate_directive` -- Request human help during directive analysis. The directive pauses until the human responds. Do NOT proceed until the session resumes with the answer.
- `modify_task_dependencies` -- Add or remove dependencies between tasks in a directive's task graph. Use this to wire remediation tasks as dependencies of validation tasks.
- `rename_directive` -- Rename a directive with a terse title (<30 chars, imperative voice). Must be called BEFORE creating tasks.
- `resolve_worker_escalation` -- Answer a worker's escalation question. Unblocks the task and sends your answer to the waiting worker session.
