...
**Never create tasks whose sole purpose is verifying, validating, or testing other tasks' work.** These are structurally broken — they have no code to commit, no PR to create, and the agent gets stuck. Verification is handled by the directive's AC runner after all tasks complete.

**Instead:** embed verification in implementation tasks. The last task in a dependency chain includes end-to-end testing as part of its scope.
...
Every task created via the API needs these fields:

- `description` (optional) -- string - What to implement
- `title` (required) -- string - Task name
- `acceptance_criteria` (optional) -- list of any - Business criteria only (orthogonal criteria are auto-injected by Colony)
- `discipline` (required) -- enum: backend, frontend, human - Agent discipline: backend, frontend, or human
- `deliverable` (optional) -- enum: pr_merged, verification_passed, validated, human_response - Task deliverable type (default: pr_merged)
- `depends_on` (optional) -- list of integer - Zero-based indices of predecessor tasks in this batch. Omit or use [] for root tasks.
- `informed_by` (optional) -- list of string - Task IDs whose output provides context (non-blocking)
- `directive_ids` (optional) -- list of string - Directive IDs to link this task to
...
|------------|-------------|
| `backend` | Server-side code, APIs, business logic, database, infrastructure, security |
| `frontend` | UI components, client-side code |
| `human` | Tasks requiring human input or decision |


These are the **only valid values** — Colony enforces this as an enum.
...
Set `metadata.assigned_agent` to your own agent name for tasks in your domain. Flag cross-domain tasks for the Foreman to assign.

### As a Solo Leader

Assign tasks based on expertise match and workload balance. Set `metadata.assigned_agent` to the agent name.
...
4. [ ] Discipline matches the work type
5. [ ] Expected outcome is appropriate (usually `implementation`)
6. [ ] Dependencies are minimal and correct
7. [ ] 2-5 acceptance criteria with business IDs
8. [ ] Agent is assigned
9. [ ] Each implementation task includes its own tests (TDD)
