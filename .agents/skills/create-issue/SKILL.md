...
argument-hint: <epic description or @path/to/epic.md>
# NOTE: Agent names in the workflow (architect-elixir, elixir-senior-developer, ui-elixir)
# are illustrative from the original HearthConnect project. The skill uses general-purpose
# subagents with role-play prompts, so agent availability does not affect execution.
synced_from_colony: true
sync_pack: universal
sync_source: packs/universal/create-issue/SKILL.md
